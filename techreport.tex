...
\begin{itemize}
\item \emph{Running time}, which is the average responding time to answer one user's application. After receiving any user $i$'s strategy $s_i$, we expect the platform will give the response $(n_i,p_i)$ as soon as possible.
...
\end{equation}
\item \emph{Completion of the sensing tasks}, which is the ratio between the number of finished tasks and the total sum of all the tasks. This ratio should be positively related to the budget $B$ as incentives.
\end{itemize}
For each item we consider two variables: the number of participating users and the budget.

\subsection{Experiment Setup}
We implemented our mechanism and the algorithm in \cite{koutsopoulos2013optimal} using Python 2.7. We ran the programs on 4-core Intel Xeon Processors with 16GB RAM, and SMP Linux with kernel version 3.19.8. Each measurement takes 1000 test samples.
