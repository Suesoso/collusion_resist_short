...
\end{equation}
Here $s^*=\kappa$, the real cost profile. {\color{blue}$s_S$ and $s_{\overline{S}}$ are the same as Definition \ref{def:gse}.}
\end{definition}
...
{\color{blue}
Note that the mechanism above captures the two most important characteristics of crowdsensing: workload (sensing time) and payment. Practical crowdsensing game may require additional restrictions. For example, each user can submit a set of sensing tasks, or the tasks are not uniform on workload, etc. Most of these restrictions are not well defined, i.e., vary among different games. In this paper, we focus on the most general typical model as above.
}
...
\end{tikzpicture}
\caption{Two cases of the parabola symmetric axis. The dashed vertical lines denote $x=k\in\mathbb{Z}$, and each solid vertical line is the symmetric axis of the parabola. Each circled point is the integer closest to the axis.}
\label{fig:quad_prop}
\end{figure}
