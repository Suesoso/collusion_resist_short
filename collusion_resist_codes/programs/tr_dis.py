# This program verifies the discrete crowdsensing game model is n-truthful.
...
f = open('loss_truthful_dis.txt', 'w')

for n_collusion in range(2, 2*n/3, (n-2)/10):
	# lost utilities of the users
	utility_loss = 0.0
	...
	for i in range(n_tests):
		...
		while changed:
			...
			for i in range(1, m):
				if len(T_k_arr[i])>0 and len(T_k_arr[i])<2:
					...
					T_k_arr[i-k] = T_k_arr[i-k] + T_k_arr[i]
					T_k_arr[i] = deepcopy(T_k_arr[i-k])
		# Let us count how much utility loss is
		u_loss = 0
		for j in range(n):
		...
		arr_u.append(u_loss*1.0 / n_collusion)
...
# new platform utility
new_u_p = R
# The outputs will be stored in the text file perturb_truthful_dis.txt
f = open('perturb_truthful_dis.txt', 'w')

for max_perturb in range(1, 11):
	# lost utility of the users
	utility_loss = 0.0
	# the standard deviation of the lost utility above
	dev_u = 0.0
	arr_u = []
	# loss of platform utility
	u_p_loss = 0.0
		...
		while changed:
			...
			for i in range(1, m):
				if len(T_k_arr[i])>0 and len(T_k_arr[i])<2:
					...
					T_k_arr[i-k] = T_k_arr[i-k] + T_k_arr[i]
					T_k_arr[i] = deepcopy(T_k_arr[i-k])
		# Let us count how much lost utility is
		u_loss = 0
		for j in range(n):
			if copy_users[j].u < users[j].u:
				u_loss = u_loss + users[j].u - copy_users[j].u
